/* Program to do link prediction using the simple product-of-degrees rule
 *
 * Written by Mark Newman  31 MAY 2024
 */
...
main(int argc, char *argv[])
{
  ...
  sprintf(filename,"degree-%06i.txt",seed);
  fp = fopen(filename,"w");
  for (u=0; u<G.n[0]; u++) {
    for (v=0; v<G.n[1]; v++) {
	...
	if (G.node[0][u].edge[i].target==v) {
      ...
      if (!exists) fprintf(fp,"%i %i %g %i\n",u,v,
		   G.node[0][u].degree*G.node[1][v].degree,removed[u][v]);
    }
  }
  fclose(fp);
}
